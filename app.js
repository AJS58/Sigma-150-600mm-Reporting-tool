...
async function generatePdf(){
  ...
  doc.roundedRect(10,10,190,36,2,2,'F');
  try {
    const logo = await imageToDataUrl('assets/cameracal-logo.png');
    doc.setFillColor(255,255,255);
    doc.roundedRect(14,14,72,28,1.5,1.5,'F');
    // Preserve the Cameracal logo aspect ratio - do not stretch/squash it.
    // Source logo aspect ratio is 3:2, so it is fitted within the white panel.
    doc.addImage(logo,'PNG',29,15,42,26);
  } catch(e) {
  ...
  doc.text('Calibration performed using Sigma USB Dock and Sigma Optimization Pro software',92,40);
  y = 51;
  ...
  sectionBar(doc,'LENS CONFIGURATION',10,y,190,'Optional'); y += 8;
  drawConfiguration(doc,y); y += 29;

  sectionBar(doc,'TECHNICIAN NOTES',10,y,190,'Optional'); y += 8;
  drawNotesBox(doc,y); y += 23;

  // Footer - fixed lower spacing to prevent overlap/clipping at the bottom of A4.
  doc.setDrawColor(...line);
  doc.line(10,265,200,265);
  doc.setFont('helvetica','bold'); doc.setFontSize(9.2); doc.setTextColor(20,20,20);
  doc.text('Thank you for trusting Cameracal Services with your equipment.', W/2, 271, {align:'center'});
  doc.setFont('helvetica','bold'); doc.setFontSize(9.5); doc.setTextColor(...blue);
  doc.text('Cameracal Services',12,279);
  doc.setFont('helvetica','normal'); doc.setFontSize(8.1); doc.setTextColor(40,40,40);
  doc.text('Professional Camera & Lens Calibration',12,284);
  doc.text('Sensor Cleaning  -  Autofocus Optimisation  -  Training',12,288);
  doc.setTextColor(...blue); doc.text('www.cameracalservices.co.uk',142,288);
...
function drawNotesBox(doc,y){
  ...
  doc.roundedRect(10,y,190,19,1.2,1.2,'FD');
  doc.setFont('helvetica','normal'); doc.setFontSize(8.3); doc.setTextColor(25,25,25);
  const notes = val('technicianNotes') || '—';
  const lines = doc.splitTextToSize(notes,182).slice(0,4);
